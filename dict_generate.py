...
font_list = ['/System/Library/Fonts/STHeiti Light.ttc',
...
class FontFilter(object):
    ...
    def __init__(self, font_file_list):
        ...
        for font_file in font_file_list:
            font = TTFont(file=font_file, fontNumber=0)
            for i in range(len(font['cmap'].tables)):
                unicode_map = font['cmap'].tables[i].ttFont.getBestCmap()
    ...
    def scan_essay(self, dict_file):
        with open(dict_file, "r") as src:
            while True:
                ...
                if len(phrase) == 1:  # 单字
                    char = phrase
                    pinyins = Pinyin(char, heteronym=False, style=Style.NORMAL, errors=returnEmpty)[0]
